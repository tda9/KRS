...
public class Authorization implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        // Get the part of the URI after the context path
        String pathAfterContext = requestURI.substring(contextPath.length());

        ////
        if (servletpath.equals("/SettingServlet")
                || servletpath.equals("/TrainingServlet")
                || servletpath.equals("/AdminUserServlet") || pathAfterContext.substring(0, 6).equals("/admin")) {
            if (daoUser.getAll("select * from User where user_name='" + name + "' and setting_id=1") != null) {
                chain.doFilter(rq, rs);
                return;
            } else {
                rs.sendRedirect("/krs/homepage.jsp");
                return;
            }
        } else {
            chain.doFilter(rq, rs);
